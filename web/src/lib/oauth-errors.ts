...
export function oauthErrorMessage(code: string, provider?: string): string {
  switch (code) {
    ...
    case 'email_exists':
      return i18n._(
        msg`An account with that email already exists. Sign in first, then link ${providerLabel(provider)} from your auth settings.`
      )
    ...
    case 'already_linked':
      return i18n._(
        msg`That ${providerLabel(provider)} account is already linked to another user`
      )
    case 'state_invalid':
