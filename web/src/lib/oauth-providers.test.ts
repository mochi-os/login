...
const methods = (
  oauth?: AuthMethodsResponse['oauth']
): AuthMethodsResponse => ({
  email: true,
  passkey: false,
  recovery: true,
...
describe('startOauth', () => {
  ...
  it('sends the validated target and the typed email, then navigates', async () => {
    ...
    expect(oauthBegin).toHaveBeenLastCalledWith('github', {
      target: '/',
      email: 'a@example.com',
    })

    await startOauth('google', { redirect: '/feeds/' })
    expect(oauthBegin).toHaveBeenLastCalledWith('google', {
      target: '/feeds/',
      email: undefined,
    })
  })
